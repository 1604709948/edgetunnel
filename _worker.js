...
// 添加需要直接使用 dailiip 的域名列表，支持从环境变量扩展
let dailidomains = ["twitch.tv", "ttvnw.net"];

const dailiIPs = ["bpb.yousef.isegaro.com"];
const cn_hostnames = [""];
let CDNIP =
  "\u0077\u0077\u0077\u002e\u0076\u0069\u0073\u0061\u002e\u0063\u006f\u006d\u002e\u0073\u0067";
// http_ip
...
export default {
  /**
   * @param {any} request
   * @param {{uuid: string, dailiip: string, cdnip: string, ip1: string, ip2: string, ip3: string, ip4: string, ip5: string, ip6: string, ip7: string, ip8: string, ip9: string, ip10: string, ip11: string, ip12: string, ip13: string, pt1: string, pt2: string, pt3: string, pt4: string, pt5: string, pt6: string, pt7: string, pt8: string, pt9: string, pt10: string, pt11: string, pt12: string, pt13: string, dailidomains: string}} env
   * @param {any} ctx
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          case `/${userID}`: {
            const laodengConfig = getlaodengConfig(
              userID,
              request.headers.get("Host")
            );
            return new Response(`${laodengConfig}`, {
              status: 200,
              headers: {
          ...
          default:
            ...
            // For any other path, reverse daili to 'ramdom website' and return the original response, caching it in the process
            ...
            // Use fetch to daili the request to 15 different domains
            const dailiUrl =
            ...
            const dailiResponse = await fetch(modifiedRequest, {
              redirect: "manual",
            });
            ...
            // Return the response from the daili server
        ...
        return await laodengOverWSHandler(request);
      }
    } catch (err) {
 ...
 * @param {any} request
 */
async function laodengOverWSHandler(request) {
  /** @type {any} */
  // @ts-ignore
        ...
        async write(chunk, controller) {
          ...
          if (isUDP) {
            if (portRemote === 53) {
              ...
              // controller.error('UDP daili only enable for DNS which is port 53');
              throw new Error("UDP daili only enable for DNS which is port 53"); // cf seems has bug, controller.error will not end stream
              return;
            }
          }
          // ["version", "附加信息长度 N"]
...
async function handleTCPOutBound(
  ...
  async function connectAndWrite(address, port) {
    ...
    const shouldUsedailiIP = dailidomains.some((domain) =>
  ...
  async function retry() {
    const tcpSocket = await connectAndWrite(
      dailiIP || addressRemote,
      dailiPort || portRemote
    );
 ...
 * @returns {string}
 */
function getlaodengConfig(userID, hostName) {
  const wlaodengws = `\u0076\u006c\u0065\u0073\u0073\u003A//${userID}\u0040${CDNIP}:8880?encryption=none&security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#${hostName}`;
  const plaodengwstls = `\u0076\u006c\u0065\u0073\u0073\u003A//${userID}\u0040${CDNIP}:8443?encryption=none&security=tls&type=ws&host=${hostName}&sni=${hostName}&fp=random&path=%2F%3Fed%3D2560#${hostName}`;
  const note = `阿杰鲁博客地址：https://ajie.lu\n阿杰鲁YouTube频道：https://www.youtube.com/@zaunist\n阿杰鲁TG电报群组：https://t.me/zaunist\n\ndailiIP全局运行中：${dailiIP}:${dailiPort}`;
    ...
    <div class="row">
			...
			<table class="table">
				...
				<tbody>
					<tr>
						...
						<td class="limited-width">${wlaodengws}</td>
						<td><button class="btn btn-primary" onclick="copyToClipboard('${wlaodengws}')">点击复制链接</button></td>
					</tr>
				</tbody>
			</table>
            <h5>客户端参数如下：</h5>
		...
		<li>传输安全(TLS)：关闭</li>
			...
			<table class="table">
				...
				<tbody>
					<tr>
						...
						<td class="limited-width">${plaodengwstls}</td>	
						<td><button class="btn btn-primary" onclick="copyToClipboard('${plaodengwstls}')">点击复制链接</button></td>
					</tr>
				</tbody>
			</table>
            <h5>客户端参数如下：</h5>
			...
			<table class="table">
					...
					<tbody>
						<tr>
							<td><button class="btn btn-primary" onclick="copyToClipboard('${wklaodengshare}')">点击复制链接</button></td>
						</tr>
					</tbody>
				</table>
    ...
    <div class="row">
        <div class="col-md-12">
			...
			<table class="table">
					...
					<tbody>
						<tr>
							<td><button class="btn btn-primary" onclick="copyToClipboard('${pglaodengshare}')">点击复制链接</button></td>
						</tr>
					</tbody>
				</table>
